import {VERSION,RELEASE_NAME,FEATURES,clamp,distance,median,Gate,BlinkTracker,RepTracker,faceMetrics,classifyHand,pairGesture,poseMetrics,imageQuality,MotionTracker,Coordinator,NoticeQueue,runSelfTests,containerNearMouth} from './analytics.mjs';
        ...
        function runBreathPhase() {
            ...
            if(state.audioUnlocked&&window.speechSynthesis){window.speechSynthesis.cancel();const utterance=new SpeechSynthesisUtterance(phase.name);utterance.lang='ru-RU';window.speechSynthesis.speak(utterance);}
...
function predict() {
 ...
 for(const [key,model,delay,process]of groups){
  ...
  if(vision.coordinator.modules.get('model-'+key)?.status==='error'){
   ...
   if(key==='face'){vision.metrics=null;lastFace=null;vision.blink.reset();state.lastFaceSeenTs=0;}if(key==='pose'){vision.rep.reset();vision.armsUp=false;}
  }
 }
...
function processFace(){
 ...
 if(vision.calibration){
  ...
  if(cal.samples.length>=20){const eyes=cal.samples.map(x=>x.eyeWidth),center=median(eyes);
   ...
   else{postureBaseline={y:median(cal.samples.map(x=>x.noseY)),eyeWidth:center,tilt:median(cal.samples.map(x=>x.tilt))};vision.calibration=null;document.getElementById('vision-calibration').textContent='Калибровка сохранена для этой сессии.';}
  }
 ...
 sensor('posture',Boolean(postureBaseline),()=>{const deviation=m.noseY-postureBaseline.y;state.currPosture=deviation>.10?'bad':'ok';document.getElementById('posture-text').textContent=deviation>.10?'Положение изменилось':'Как при калибровке';
  document.getElementById('posture-bar').style.width=`${clamp(100-Math.abs(deviation)*300,10,100)}%`;
  if(vision.gates.posture.update(deviation>.10,now)){state.postureCorrections++;sensorEvent('posture','Положение головы изменилось. Проверьте, удобно ли вы сидите.','↕️');updateStatsUI();}});
...
function processHands(){
 ...
 sensor('water',available&&Boolean(mouth),()=>{document.getElementById('dbg-status').textContent=drinks?'Ёмкость у рта':'Наблюдение';
  if(vision.gates.water.update(drinks&&!pair,now)){document.getElementById('water-confirm').hidden=false;sensorEvent('water','Возможный напиток: подтвердите объём вручную.','💧');}});
...
function dispatchNotice(){
 ...
 if(state.audioUnlocked&&window.speechSynthesis){const utterance=new SpeechSynthesisUtterance(item.text);utterance.lang='ru-RU';utterance.rate=.95;window.speechSynthesis.speak(utterance);}
}
