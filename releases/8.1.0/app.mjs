...
import {VERSION,RELEASE_NAME,FEATURES,clamp,distance,median,Gate,BlinkTracker,RepTracker,faceMetrics,classifyHand,pairGesture,poseMetrics,imageQuality,MotionTracker,Coordinator,NoticeQueue,runSelfTests,containerNearMouth} from './analytics.mjs';
        ...
        function runBreathPhase() {
            ...
            if(state.audioUnlocked&&window.speechSynthesis){window.speechSynthesis.cancel();const utterance=new SpeechSynthesisUtterance(phase.name);utterance.lang='ru-RU';utterance.rate=habits.settings.voiceRate;utterance.volume=habits.settings.voiceVolume;window.speechSynthesis.speak(utterance);}
...
const vision=createVisionRuntime();const habits=createHabits();
...
function dispatchNotice(){
 const blocked=document.hidden||vision.breathing||state.eyeBreakActive||vision.privacyActive||!vision.settings.notifications||(vision.settings.quietFocus&&pomoState.running&&!pomoState.isBreak)||window.speechSynthesis?.speaking;
 const item=vision.notices.next(Date.now(),blocked);if(!item)return;toast(item.text);
 if(state.audioUnlocked&&window.speechSynthesis){const utterance=new SpeechSynthesisUtterance(item.text);utterance.lang='ru-RU';utterance.rate=habits.settings.voiceRate;utterance.volume=habits.settings.voiceVolume;window.speechSynthesis.speak(utterance);}
}
...
function renderHabits(){const today=habits.log.filter(e=>localDay(e.time)===localDay());document.getElementById('habit-drinks').textContent=`Стакан/кружка: ${today.filter(e=>e.type==='cup').length} · бутылка: ${today.filter(e=>e.type==='bottle').length} · всего: ${today.length}`;document.getElementById('habit-confirmed').textContent=`Подтверждено вручную: ${waterTotal()} мл`;document.getElementById('habit-voice-status').textContent=state.audioUnlocked?'Голос включён':'Нажмите «Включить голос»';document.getElementById('habit-drink-history').textContent=habits.log.slice(-12).reverse().map(e=>`${new Date(e.time).toLocaleString('ru-RU')} — ${e.type==='cup'?'Стакан/кружка':'Бутылка'} у рта`).join('\n')||'Поднесений пока нет.';}
...
function initHabits(){for(const key of Object.keys(HABIT_DEFAULTS)){const el=document.getElementById('habit-'+key);if(!el)continue;if(typeof HABIT_DEFAULTS[key]==='boolean')el.checked=habits.settings[key];else el.value=habits.settings[key];}
 ...
 document.getElementById('habit-enable-voice').onclick=()=>{if(!window.speechSynthesis){toast('Браузер не поддерживает озвучку.');return;}state.audioUnlocked=true;audioBtn.classList.add('unlocked');document.getElementById('audioStatusIcon').innerText='🔊';document.getElementById('audioStatusText').innerText='ЗВУК ЕСТЬ';vision.settings.notifications=true;vision.settings.quietFocus=false;habits.settings.drinkVoice=true;document.getElementById('habit-drinkVoice').checked=true;document.getElementById('vision-notifications').checked=true;document.getElementById('vision-quiet-focus').checked=false;saveHabits();const u=new SpeechSynthesisUtterance('Голос включён. Я сообщу, когда стакан или бутылка окажется у рта.');u.lang='ru-RU';u.rate=habits.settings.voiceRate;u.volume=habits.settings.voiceVolume;window.speechSynthesis.cancel();window.speechSynthesis.speak(u);renderHabits();};
